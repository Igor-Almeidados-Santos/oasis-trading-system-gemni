...
def test_list_metrics_applies_strategy_filter() -> None:
    db = RecordingDB([[_metric_row()]])
    service = TelemetryService(db)

    service.list_metrics(strategy_id="xyz")

    recorded = db.calls[0]
    normalized_query = " ".join(recorded["query"].split())
    assert "m.strategy_id = %s" in normalized_query
    assert recorded["params"] == ("xyz",)


def test_list_metrics_applies_timeframe_filter() -> None:
    db = RecordingDB([[_metric_row()]])
    service = TelemetryService(db)

    service.list_metrics(timeframe="7d")

    recorded = db.calls[0]
    normalized_query = " ".join(recorded["query"].split())
    assert "m.timeframe = %s" in normalized_query
    assert recorded["params"] == ("7d",)


def test_list_metrics_combines_filters_in_param_order() -> None:
    since = datetime(2024, 3, 15, tzinfo=timezone.utc)
    db = RecordingDB([[_metric_row()]])
    service = TelemetryService(db)

    service.list_metrics(since=since, strategy_id="xyz", timeframe="24h")

    recorded = db.calls[0]
    normalized_query = " ".join(recorded["query"].split())
    assert "WHERE m.computed_at >= %s AND m.strategy_id = %s AND m.timeframe = %s" in normalized_query
    assert recorded["params"] == (since, "xyz", "24h")
