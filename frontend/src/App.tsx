...
const TIMEFRAME_OPTIONS: MetricTimeframe[] = ['1h', '24h', '7d', '30d'];
const TIMEFRAME_WINDOWS: Record<MetricTimeframe, number> = {
  '1h': 2,
  '24h': 14,
  '7d': 60,
  '30d': 180,
...
function App() {
  ...
  const [selectedTimeframe, setSelectedTimeframe] = useState<MetricTimeframe>('24h');
  ...
  const fetchData = useCallback(
    async ({ initial = false } = {}) => {
      ...
      try {
        const windowDays = TIMEFRAME_WINDOWS[selectedTimeframe] ?? 30;
        const since = new Date(Date.now() - windowDays * 24 * 60 * 60 * 1000).toISOString();
        const strategyFilter = selectedStrategy !== 'all' ? selectedStrategy : undefined;

        const [summaryData, positionsData, strategiesData, metricsData, exchangesData] =
          await Promise.all([
            getPortfolioSummary(),
            getPortfolioPositions(),
            getStrategies(),
            getTelemetryMetrics({ since, timeframe: selectedTimeframe, strategyId: strategyFilter }),
            getExchangeBreakdown(),
          ]);
        ...
        if (
          ...
          !strategiesData.some((strategy) => strategy.id === selectedStrategy)
        ) {
    ...
    [selectedStrategy, selectedTimeframe],
  );
  ...
  const selectedStrategyName = useMemo(() => {
    if (selectedStrategy === 'all') {
      return 'Todas as estratégias';
    }
    const match = strategies.find((strategy) => strategy.id === selectedStrategy);
    return match ? match.name : 'Todas as estratégias';
  ...
  const handleTimeframeSelect = useCallback((timeframe: MetricTimeframe) => {
            ...
            <section className="card">
              <div className="card-header">
                <div>
                  ...
                  <p className="card-subtitle">
                    Retornos líquidos e drawdown na janela {selectedTimeframe.toUpperCase()}
                    {selectedStrategy !== 'all' ? ` • ${selectedStrategyName}` : ''}
                  </p>
                </div>
                <div className="card-actions">
                    ...
                    <div className="timeframe-toggle">
                      {TIMEFRAME_OPTIONS.map((option) => (
                        <button
                          key={option}
                          type="button"
                          className={`toggle-button ${selectedTimeframe === option ? 'active' : ''}`}
                  ...
                  {strategies.length > 0 && (
                    <div className="filter-group">
                      <label className="filter-label" htmlFor="strategy-filter">
                        Estratégia
                      </label>
                      <select
                        id="strategy-filter"
              ...
              {metrics.length ? (
                <div className="table-wrapper">
                  <table className="data-table">
                    ...
                    <tbody>
                      {metrics.map((metric) => (
                        <tr key={`${metric.strategyId}-${metric.timeframe}-${metric.computedAt}`}>
                          <td>{metric.name}</td>
                ...
                <div className="empty-state">
                  {selectedStrategy === 'all'
                    ? 'Sem métricas registradas para o período selecionado.'
                    : 'Sem métricas para esta estratégia no período selecionado.'}
                </div>
              )}
            </section>
          </>
