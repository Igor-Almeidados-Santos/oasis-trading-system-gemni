...
async def get_telemetry_metrics(
    ...
    strategy_id: Optional[str] = Query(
        None,
        alias="strategyId",
        description="Filtra métricas de uma estratégia específica.",
    ),
    timeframe: Optional[str] = Query(
        None,
        description="Limita os resultados a um timeframe específico (ex: 1h, 24h, 7d, 30d).",
    ),
    service: TelemetryService = Depends(get_telemetry_service),
) -> list[StrategyMetric]:
    return await run_in_threadpool(service.list_metrics, since, strategy_id, timeframe)
